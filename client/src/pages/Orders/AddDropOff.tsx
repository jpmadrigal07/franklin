    ...
    if (selectedAddOnService && JSON.parse(selectedAddOnService)._id) {
      ...
      MySwal.fire({
        ...
        if (saveType === "saveEnd") {
          ...
          navigate("/orders/dropoff");
        } else if (saveType === "saveAdd") {
          window.location.href = `/orders/dropoff/add?customerId=${
  ...
  const _addOrder = (type: string) => {
    ...
    const customDiscountsTotal =
      selectedDiscounts.length > 0
        ...
        : 0;
    const dropOffFee = laundryData[0] ? laundryData[0]?.price : 0;
    const amountDue =
      washTotal +
      dryTotal +
      detergentTotal +
  ...
  return (
    <>
      <div className="flex justify-between mt-11">
        <div>
          <h1 className="font-bold text-primary mt-7">Add Drop-Off Order</h1>
        </div>
...
export default connect(mapStateToProps)(AddDropOff);
